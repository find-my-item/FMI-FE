...
import useApiTokenRefresh from "./api/fetch/auth/api/useApiTokenRefresh";
...
export function middleware(request: NextRequest) {
  ...
  const { mutate: tokenRefreshMutate } = useApiTokenRefresh();
  ...
  if (!accessToken && refreshToken) {
    // 리프레쉬 토큰으로 토큰 재발급
    tokenRefreshMutate(undefined, {
      onSuccess: (res) => console.log("res>>> ", res),
      onError: (error) => {
        console.log("error>> ", error);
        if (error.code === "AUTH401-INVALID_REFRESH") {
        }
      },
    });
  }
